...
(attrName=(literal)) ':' (attrValue=expression) {
  $key = $attrName.value; $value = $attrValue.value;
};

expression returns [String value]:
//literal {$value = $literal.value;} |
conditionalExpression {$value = $conditionalExpression.text;}
...
unaryExpressionNotPlusMinus 
    :   '~' unaryExpression
    |   '!' unaryExpression
    |   castExpressionOrPrimaryParExpression
    |   primaryNonParExpression
        (selector)*
        ('++' |   '--')?
    ;
   

/** Generalized to be nice to ANT. -- Ray Myers
castExpression 
...
identifierSuffix 
    :  (('[')=> '[' expression? ']')+ ('.' 'class')?
    |   arguments /* Generalized -- Ray Myers*/
...
selector  
    ...
    |   '[' expression ']'
    ;
// Generalized to be nice to ANTLR -- Ray Myers
creator 
    :   'new' 
...
classCreatorRest 
    :   arguments
        // (classBody)? 
        // Jaml currently disallows inner classes in attribute hashes.
        // I mean come on, man.
    ;
...
LPAREN : { !textMode }?=> '(';
RPAREN : { !textMode }?=> ')';
LBRACE: { !textMode }?=>'{' {hashMode = true; braceDepth++};
RBRACE: { hashMode }?=>'}' {if (--braceDepth == 0) hashMode = false; /*textMode = true;*/};
LBRACKET : { !textMode }?=> '[';
RBRACKET : { !textMode }?=> ']';
...
LONGLITERAL : { !textMode }?=> IntegerNumber LongSuffix;

    
INTLITERAL : { !textMode }?=>   IntegerNumber;
...
FLOATLITERAL : { !textMode }?=> NonIntegerNumber FloatSuffix;

DOUBLELITERAL : { !textMode }?=> NonIntegerNumber DoubleSuffix?
    ;

CHARLITERAL : { !textMode }?=> 
		'\'' 
        (   EscapeSequence 
        |   ~( '\'' | '\\' | '\r' | '\n' )
        ) 
...
IDENTIFIER : { !textMode }?=> IdentifierStart IdentifierPart*;
